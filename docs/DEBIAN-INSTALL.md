...
## عیب‌یابی: چراغِ «زمان‌بندِ سرور» در صفحهٔ پشتیبان‌گیری قرمز است

این چراغ فقط وقتی سبز می‌شود که زمان‌بندِ سیستم (`schedule:run`) هر دقیقه اجرا شود
و یک «ضربان» در دیتابیس بنویسد. بکاپِ خودکار هم به همین وابسته است. اگر قرمز است،
به ترتیب این‌ها را بررسی کن (اسکریپتِ نصب هم پایانِ کار همین‌ها را خودکار چک می‌کند):

1. **اتصالِ دیتابیس** — رایج‌ترین علت. باید بدونِ خطا اجرا شود:
   ```bash
   cd /var/www/soorin-inventory
   sudo -u www-data php artisan migrate:status
   ```
   اگر «Access denied» داد، رمزِ `.env` با کاربرِ MySQL هماهنگ نیست یا کشِ پیکربندی
   کهنه است. کش را پاک کن و دوباره امتحان کن:
   ```bash
   sudo -u www-data php artisan optimize:clear
   ```
   اجرای دوبارهٔ `sudo bash deploy/debian-setup.sh` هم رمزِ کاربرِ MySQL را با `.env`
   هماهنگ می‌کند (بدون پاک‌کردنِ داده).

2. **تایمرِ زمان‌بند** — باید `active` باشد:
   ```bash
   systemctl status soorin-scheduler.timer
   ```
   اگر نبود، نصبش کن:
   ```bash
   sudo bash deploy/install-scheduler.sh
   ```
   این اسکریپت پس از نصب، خودش اتصالِ دیتابیس را می‌سنجد و اگر مشکلی باشد می‌گوید.

> نکته: صرفِ نصبِ تایمر کافی نیست؛ اگر دیتابیس در دسترس نباشد، تایمر می‌دود ولی
> ضربان نوشته نمی‌شود و چراغ قرمز می‌ماند. اول قدم ۱ (دیتابیس) را درست کن.
